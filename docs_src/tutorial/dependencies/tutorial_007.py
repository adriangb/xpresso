...
from xpresso import App, Dependant, FromPath, FromQuery, HTTPException, Operation, Path


def require_roles(*roles: str) -> Callable[..., None]:
    role_set = frozenset(roles)

    def enforce_roles(roles: FromQuery[Optional[FrozenSet[str]]] = None) -> None:
        missing_roles = role_set.difference(roles or frozenset())
        if missing_roles:
            raise HTTPException(403, f"Missing roles: {list(missing_roles)}")
    ...
    routes=[
        Path(
            ...
            delete=Operation(
                ...
                dependencies=[Dependant(require_roles("items-admin"))],
            ),
