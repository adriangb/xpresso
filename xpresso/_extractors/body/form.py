...
from xpresso._extractors.api import BodyExtractor, BodyExtractorMarker
from xpresso._extractors.body.form_field import FormFieldBodyExtractorMarker
from xpresso._extractors.validator import validate as validate_data
from xpresso._param_dependants import BodyDependantMarker
...
class FormDataBodyExtractorMarkerBase(BodyExtractorMarker):
    ...
    def register_parameter(self, param: inspect.Parameter) -> BodyExtractor:
        ...
        for param_name, field_param in inspect.signature(annotation).parameters.items():
            marker: typing.Optional[BodyDependantMarker] = None
            for param_marker in get_markers_from_parameter(field_param):
                if isinstance(param_marker, BodyDependantMarker):
                    marker = param_marker
                    break
